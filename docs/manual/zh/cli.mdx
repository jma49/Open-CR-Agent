...
| `--no-repo-config` | 忽略 `.ocra/config.json` 及其中列出的插件，模型改从 `OCRA_MODEL_*` 读取。审查不信任的代码时使用 |
| `-h, --help` | 显示帮助 |
...
`--format json` 会输出完整报告：变更信息、风险档位、每个改动文件的覆盖情况（已审查、失败、未被覆盖，或被排除及原因）、分组、各任务结果、被 Matrix 跳过的（审查员，分组）组合、带行号的问题、token 用量与花费，以及警告信息。
